...
i = 0
while x_reps == -1 || y_reps == -1 || z_reps == -1

  if x_reps == -1
    current_x_state = moons.collect {|moon| [moon[0], moon[3]]}
    if x_history.member? current_x_state
      x_reps = i
    else
      x_history[current_x_state] = i
    end
  end
  if y_reps == -1
    current_y_state = moons.collect {|moon| [moon[1], moon[4]]}
    if y_history.member? current_y_state
      y_reps = i
    else
      y_history[current_y_state] = i
    end
  end
  if z_reps == -1
    current_z_state = moons.collect {|moon| [moon[2], moon[5]]}
    if z_history.member? current_z_state
      z_reps = i
    else
      z_history[current_z_state] = i
    end
  end
...
puts [x_reps, y_reps, z_reps].reduce(1, :lcm)
